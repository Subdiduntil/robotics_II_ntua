...
class xArm7_kinematics():
    ...
    def compute_jacobian(self, r_joints_array):
        ...
        # Links of Robot and Angles theta1, theta2
        l1  = self.l1 # unused variable
        l2  = self.l2
        l3  = self.l3
        l4  = self.l4
        l5  = self.l5
        ...
        s5 = np.sin(q5)
        s6 = np.sin(q6)
        s7 = np.sin(q7) # unused variable

        ################## 1st Line of Jacobian Matrix - x coordinates ##################
        ...
        ################## 2nd Line of Jacobian Matrix - y coordinates ##################
        ...
        ################## 3rd Line of Jacobian Matrix - z coordinates ##################
    ...
    def tf_A05(self, r_joints_array):
        ...
        tf_A45 = np.matrix([[c5 , -s5 , 0 ,  l4 * np.sin(th1)],\
                           [  0 ,  0  ,-1 , -l4 * np.cos(th1)],\
                           [ s5 ,  c5 , 0 ,  0],\
                           [0   ,  0  , 0 ,  1]])
    ...
    def tf_A07(self, r_joints_array):
        ...
        tf_A67 = np.matrix([[c7 , -s7 , 0 ,  l5 * np.sin(th2)],\
                           [  0 ,  0  , 1 ,  l5 * np.cos(th2)],\
                           [-s7 , -c7 , 0 ,  0],\
                           [0   ,  0  , 0 ,  1]])
